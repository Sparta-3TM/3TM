...
public class NaverService {
    ...
    public List<StopoverDto> naverApi(String[] str, int stopoverNum) throws JsonProcessingException {
        ...
        System.out.println("최종 목적지까지의 거리 = " + summary.getGoal().getDistance() / 1000D + "km");
        System.out.println("최종 목적지까지의 소요시간 = " + summary.getGoal().getDuration() / 1000D / 3600 + "시간");

        result.add(new StopoverDto((summary.getGoal().getDistance() - distance) / 1000D, (summary.getGoal().getDuration() - duration) / 1000D / 3600 ));
        result.add(new StopoverDto(summary.getGoal().getDistance() / 1000D, summary.getGoal().getDuration() / 1000D / 3600));
        return result;
    }
